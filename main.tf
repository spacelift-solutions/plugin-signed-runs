...
resource "github_repository_file" "workflow" {
  ...
  repository = data.spacelift_stack.this[each.key].repository
  file       = ".github/workflows/spacelift_signed_run_${trim(replace(replace(replace(each.value.stack_id, "*", ""), "/", "_"), "-", "_"), "_")}${trim(replace(replace(replace(each.value.path, "*", ""), "/", "_"), "-", "_"), "_")}.yaml"
